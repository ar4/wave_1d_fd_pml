...
def find_profile(profile, bounds, optimizer, init=None, vs=[1500], pml_version=2, dx=5, maxiter=500):
    dt = 0.0006 * dx / 5
    N = int(100 * 5 / dx)
    ...
    for v0 in vs:
        for v1 in vs:
            models=[test_wave_1d_fd_pml.model_one(nsteps, v0=v0, v1=v1, dx=dx, dt=dt, N=N)]
    ...
    if optimizer == 'hygsa':
        ...
        ret = scipy.optimize.brute(cost, bounds, Ns=int(maxiter ** (1/len(bounds))))
        xmin = ret
        fxmin = cost(ret)
    else:
        raise ValueError('unknown optimizer')
    print(optimizer, 'pml ', pml_version, ' global minimum: xmin = {0}, f(xmin) = {1}'.format(xmin, fxmin))
